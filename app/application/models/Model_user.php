...
class Model_user extends CI_Model{
	...
	{
		return $this->db->query("SELECT * FROM saw_users WHERE NOT EXISTS (SELECT * FROM saw_alternatives WHERE saw_users.id_user = saw_alternatives.id_user AND id_event = '$id') AND id_user != '1' AND saw_users.status = 'on'");
	}
		public function getKandidatAll($id){
		return $this->db->query("SELECT * FROM saw_users WHERE NOT EXISTS (SELECT * FROM saw_login WHERE saw_users.id_user = saw_login.id_user AND id_event = '$id') AND id_user != '1' AND saw_users.status = 'on'"); 		
		// $this->db->select('*');
		// $this->db->from($this->tabel);
		// $this->db->join('saw_login', 'saw_login.id_user = '.$this->tabel.'.id_user', 'left');
		// $this->db->join('saw_access', 'saw_access.id_access = saw_login.id_access', 'left');
